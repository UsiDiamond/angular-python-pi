...
nohup gunicorn --access-logfile logs/access.log --error-logfile logs/error.log --certfile=server.crt --keyfile=server.key --bind 0.0.0.0:9443 -w 4 app:hello   &  nohup node server.js 